...
## Status as of 16.07.2026: fully landed, PR #1 closed

Confirmed via `gh pr view 1` (16.07.2026): PR #1 is `CLOSED`, `mergedAt: null`
— i.e. closed manually rather than merged through GitHub, which is expected:
the fix landed on `etap2` directly as commit `f6d673f` (same diff as the PR's
source commit `378a5f0` on the now-gone worktree branch, but a fresh commit,
not a GitHub merge/cherry-pick — that's *why* GitHub never auto-closed the
PR). `closedAt: 2026-07-07T20:33:59Z` — same day as the fix itself, so this
was already fully wrapped up on 07.07; the "PR #1 needs manual close" note
that persisted in `MEMORY.md` afterward was simply never updated once the
close actually happened. No outstanding action here.

## Status as of 06.07.2026 (end of session) — superseded, kept for history

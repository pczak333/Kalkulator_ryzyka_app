...
├── doc_splitter.py      # Segments multi-page PDF into logical documents via _classify_page_segment(); rule order (CRITICAL): Rule 0 (KOD [hash] — EPU form code, only on EPU header, never in uzasadnienie) → Rule 1 (art.299→pozew) → Rule 5a (P O Z E W before Rule 2!) → Rule 2/2b (NAKAZ at line-start (?m)^\s* within 200 chars — strict to avoid false-positives from pozew body text) → Rule 2d (UZASADNIENIE guard→None) → Rule 3 (wezwanie) → Rule 4 (POUCZENIE→None) → Rule 5b (POZEW) → Rule 6 (fallback); Rule 2c REMOVED (was main source of false-positives for uzasadnienie pages)
...
- `Lublin_pozew_nak._zap..pdf` — wielostronicowy PDF (str.1: pozew EPU "P O Z E W" + KOD [hash], str.2: pismo przewodnie sądu, str.3-4: uzasadnienie z ref. do Km 990/21 i Woodraft Home Sp. z o.o., str.5-7: nakaz EPU + KOD [hash], str.8: pusta); powód: Krzysztof Knop; pozwany: Piotr Czak PESEL 61010501435; sygnatura: VI Nc-e 222431/23; kwota: 5 267,77 zł; expected po commit 6f112d2: [1/2] nakaz str.5-7 GŁÓWNY + [2/2] pozew str.1-4 POMOCNICZY, K1=Nakaz (członek zarządu), termin 14 dni, bramka art.299 TAK; DO WERYFIKACJI (niezdane po 6f112d2)

...
├── doc_selector.py      # Scores candidates (CSV 02) + tie-breaking (CSV 04) → main document; hard rules: (1) any NAKAZ always wins over POZEW; (2) among nakazy, nakaz WITH deadline_days wins over nakaz without (prevents misclassified uzasadnienie pages from beating actual EPU nakaz); R17 scoring fires for deadline_days alone (no delivery_date needed)

...
├── doc_splitter.py      # Segments multi-page PDF into logical documents via _classify_page_segment(); rule order (CRITICAL): Rule 0 (KOD [hash] — EPU form code, only on EPU header, never in uzasadnienie) → Rule 1 (art.299→pozew) → Rule 5a (P O Z E W before Rule 2!) → Rule 5a' (P O Z E W anywhere) → Rule 2d (UZASADNIENIE guard→None) → Rule 2/2b (NAKAZ at line-start (?m)^\s* within 200 chars — strict to avoid false-positives from pozew body text) → Rule 3 (wezwanie) → Rule 4 (POUCZENIE→None) → Rule 5b (POZEW, full text) → Rule 6 (fallback); Rule 2c REMOVED; post-processing: Krok 0 upgrades unknown→pozew if [unknown→nakaz_noKOD→nakaz_KOD]; Krok 1 nuclear: merges all pre-KOD segments into pozew when pre-block has pozew+other (handles evidence-table pages misclassified as wezwanie). KEY INSIGHT: Lublin_pozew_nak._zap..pdf is SCANNED — pdfplumber=0 text, Azure DI does OCR; str.4 (evidence table) has "Wezwanie do zapłaty" as evidence title → Rule 3 fires → wezwanie segment; nuclear merge fixes this.
├── doc_selector.py      # Scores candidates (CSV 02) + tie-breaking (CSV 04) → main document; hard rules: (1) any NAKAZ always wins over POZEW; (2) among nakazy, nakaz WITH deadline_days wins over nakaz without (prevents misclassified uzasadnienie pages from beating actual EPU nakaz); R17 scoring fires for deadline_days alone (no delivery_date needed); bundle upgrade: SPOLKA→CZLONEK_ZARZADU when art.299 OR "czlonek zarządu" found in any bundle doc
...
- `Lublin_pozew_nak._zap..pdf` — SKANOWANY wielostronicowy PDF (pdfplumber=0 tekstu, Azure DI robi OCR); str.1: pozew EPU (KOD NIE wykrywany przez Azure DI na str.1!), str.2: pismo przewodnie sądu, str.3: uzasadnienie z art.299, str.4: tabela dowodów EPU (zawiera "Wezwanie do zapłaty" w liście → Rule 3 klasyfikowała jako wezwanie), str.5-7: nakaz EPU (KOD wykrywany przez Azure DI); powód: Krzysztof Knop; pozwany: Piotr Czak PESEL 61010501435; sygnatura: VI Nc-e 222431/23; kwota: 5 267,77 zł; expected po commit a67f6e3: [1/2] nakaz str.5-7 GŁÓWNY + [2/2] pozew str.1-4 POMOCNICZY — ZWERYFIKOWANE na prawdziwym OCR Azure DI (detect_documents_by_pages zwraca 2 segmenty); DO WERYFIKACJI w Streamlit
- `obraz1.png` — screenshot 30.06.2026 (po d26823c, przed Bug E fix — nakaz w [3/3] aux, kwota 3 690 błędna)

...
- `art299_pozew_nakaz_umorz._egzek..pdf` — nowy plik testowy (02.07.2026), 13 str. skan; Sygn. XIII I GNc 6937/20, Sąd Rejonowy dla Łodzi-Śródmieścia; powód: Kiloutou Polska sp. z o.o.; pozwany/dłużnik: Woodraft Home sp. z o.o.; kwota nakazu 4 547,81 zł. Struktura: str.1-6 pozew, str.7-8 nakaz zapłaty + klauzula wykonalności (12.04.2021), str.9-11 wniosek wierzyciela o wszczęcie postępowania egzekucyjnego do komornika, str.12-13 formalne POSTANOWIENIE komornika (sygn. GKm 46/21, Jakub Niedopytalski) o umorzeniu postępowania egzekucyjnego z urzędu (art. 824 §1 pkt 3 k.p.c. — brak majątku dłużnika = bezskuteczność egzekucji, przesłanka art. 299 KSH). **STATUS 02.07.2026: ROZWIĄZANE I ZWERYFIKOWANE.** Dwa niezależne błędy naprawione: (1) segmentacja stron w `doc_splitter.py` — nowe reguły 1b/1c/4b (patrz wyżej), nakaz teraz poprawnie str.7-8, wniosek egzekucyjny i postanowienie o umorzeniu poprawnie wykryte jako osobne segmenty; (2) finalna klasyfikacja w `doc_classifier.py` — bonus adresata rozszerzony o typy bez sufiksu `_SPOLKA`/`_CZLONEK_ZARZADU` (patrz wyżej), plus dostrojone słowa kluczowe/sygnały silne dla `UMORZENIE_EGZEKUCJI_BEZSKUTECZNOSC` i `WNIOSEK_EGZEKUCYJNY` w CSV 07 (realistyczne dopasowanie do faktycznej sentencji, nie sparafrazowany opis). Zweryfikowane end-to-end DWUKROTNIE (stabilność wobec wariancji OCR między uruchomieniami): main=`NAKAZ_CZLONEK_ZARZADU` str.7-8 (bundle-upgrade z NAKAZ_SPOLKA — bundle zawiera wzmiankę o członku zarządu), aux=`POZEW_CZLONEK_ZARZADU` str.1-6, `WNIOSEK_EGZEKUCYJNY` str.9-11, `UMORZENIE_EGZEKUCJI_BEZSKUTECZNOSC` str.12-13. Regresja: 6/6 wcześniej znanych plików testowych nadal przechodzi bez zmian (zweryfikowane po obu naprawach).

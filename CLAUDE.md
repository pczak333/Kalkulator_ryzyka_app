...
│   - **(04.08.2026, po audycie przed publikacją strony) Cztery zmiany, których NIE cofać bez zrozumienia powodu:**
│     **(a) Nieaktualny wynik.** `_state_now` (migawka bieżących odpowiedzi) liczy się przy KAŻDYM przeładowaniu i jest porównywana z zapisaną w `krs_answers`/`krs_epu`/`krs_days_exact`. Różnica ⇒ kasujemy zapisany wynik i prosimy o ponowne przeliczenie. Wcześniej sekcja wyniku renderowała się ze starej migawki także po zmianie formularza — klient widział nieaktualną ocenę ryzyka i mógł pobrać niezgodny PDF, bez żadnego ostrzeżenia.
│     **(b) Wyrok zaoczny przy ręcznym formularzu.** `_K1_IMPLIES_DOC_TYPE` sprawia, że kod K1 wskazany przez klienta sam wyznacza `DOC_TYPE` (ma pierwszeństwo przed rozpoznaniem OCR). Bez tego podmiana tekstów wyroku zaocznego odpalała się WYŁĄCZNIE po wgraniu pliku, więc klient wypełniający ręcznie dostawał raport o nakazie zapłaty i poradę „sprzeciw albo zarzuty" (przy wyroku zaocznym jedyną drogą jest sprzeciw).
...
├── hard_rules.py        # Applies hard safety rules (5B_Twarde_reguly) that override score; HR10 (low OCR quality) + HR11 (pismo procesowe — incomplete docs) active. HR02/HR04 (pilność 0-3/4-7 dni dla nakazu-członka-zarządu) sprawdzają `K1 in _NAKAZ_LIKE_CZLONEK_CODES` (tuple, nie pojedynczy literał) — obejmuje też `K1_WYROK_ZAOCZNY_CZLONEK_ZARZADU`, bo rozdzielenie kodów K1 wyroku/nakazu inaczej po cichu wyłączyłoby te reguły bezpieczeństwa. (04.08.2026) Reguły mówiące o pozostałym czasie (HR01/HR02/HR06) mają pole `warning_passed` — alternatywne brzmienie używane, gdy `state["DEADLINE_PASSED"]` jest ustawione przez app.py. Poziom ryzyka bez zmian. Warianty udokumentowane też w CSV 11 (kolumna `uwagi_dla_AI`); UWAGA: `data_loader.load_hard_rules()` jest zdefiniowane, ale NIGDZIE nie wywoływane — reguły są zaszyte w tym pliku, a CSV 11 pełni rolę dokumentacji, więc **zmieniając regułę trzeba ręcznie zaktualizować oba miejsca**.
...
├── text_builder.py      # Assembles final output text; sanitize_check() verifies no codes leak. `_LEGAL_DISCLAIMER` i 5 wariantów CTA (`_CTA_PERSONAL/_COMPANY/_UNKNOWN/_PISMO_PROCESOWE/_ZUS`) są HARDKODOWANE tutaj (nie w CSV) — jedyne miejsce do edycji tych tekstów. Disclaimer/CTA renderowane przez `report_builder.py` z `output["disclaimer"]`/`output["cta"]` przez `markup_bold`. (04.08.2026) Ostrzeżenia o WIARYGODNOŚCI danych (HR10 — niepewny odczyt, HR11 — brak pełnych akt) są wyciągane PRZED pozostałe i pokazywane zawsze; reszta nadal ograniczona do dwóch. Wcześniej raport brał po prostu `warnings[:2]`, a HR10/HR11 są dopisywane na końcu listy w `hard_rules.apply()` — więc w typowej sprawie z krótkim terminem (2 ostrzeżenia o pilności) klient NIGDY nie dowiadywał się, że dokument mógł zostać źle odczytany. Parowanie `triggered`↔`warnings` po indeksie, z bezpiecznym fallbackiem gdyby listy się rozjechały.
│
...
```
tools/
├── smoke_test_ui.py         # **(04.08.2026) Szybki test dymny interfejsu BEZ przeglądarki** — `python tools/smoke_test_ui.py` (kod 0/1). Renderuje aplikację przez `streamlit.testing.v1.AppTest` i przechodzi ścieżkę: start → wypełnienie formularza → „Oblicz ryzyko" → „Wyczyść kalkulator". Łapie `StreamlitAPIException` (modyfikacja klucza widżetu po jego utworzeniu, zagnieżdżone expandery) — czyli klasę błędów, które wywalają CAŁĄ aplikację czerwoną ramką, a których `regression_test.py` nie widzi (nie renderuje UI). **URUCHAMIAĆ PO KAŻDEJ ZMIANIE w app.py dotykającej widżetów, session_state albo kontenerów.** Powód powstania: błąd „Wyczyść kalkulator" przeżył w kodzie od czerwca do sierpnia, bo jedynym sposobem wykrycia było ręczne klikanie. To **koryguje wcześniejszą notatkę** (`memory/project_progress_indicator.md`), że takie błędy wymagają żywej przeglądarki — AppTest wystarczy i trwa kilkanaście sekund. Dwie pułapki: (1) `labeled_radio()` używa `options=range(N)`+`format_func`, więc `set_value(0)`, NIE `set_value(r.options[0])`; (2) po zniknięciu sekcji wyniku Streamlit sprząta stan jej widżetów, a AppTest trzyma stare drzewo — nie zapisywać widżetów po takim przebiegu (KeyError w samym AppTest, nie błąd aplikacji).
